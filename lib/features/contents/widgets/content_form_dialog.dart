...
const double _compactDialogPadding = 12.0;
const double _standardDialogPadding = 24.0;
...
class _ContentFormDialogState extends State<ContentFormDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    return Dialog(
      insetPadding: EdgeInsets.symmetric(
        horizontal: isCompact ? _compactDialogPadding : _standardDialogPadding,
        vertical: isCompact ? _compactDialogPadding : _standardDialogPadding,
      ),
      ...
      child: ConstrainedBox(
        ...
        child: Padding(
          padding: EdgeInsets.all(isCompact ? 16 : _standardDialogPadding),
          child: Form(
            key: _formKey,
            child: SingleChildScrollView(
              child: Column(
                ...
                children: <Widget>[
                  ...
                  if (_isPostType) ...<Widget>[
                    const SizedBox(height: 18),
                    _buildPostFields(isCompact: isCompact),
                  ],
                  const SizedBox(height: 24),
                  _buildFooterActions(isCompact: isCompact, isEdit: isEdit),
                ],
              ),
            ),
          ),
  ...
  Widget _buildTypeAndStatusFields({
    ...
    final List<DropdownMenuItem<String>> typeOptions = _contentTypes
        .map(
    ...
    final List<DropdownMenuItem<String>> statusOptions = _contentStatuses
        .map(
    ...
    final DropdownButtonFormField<String> typeField =
    ...
    final DropdownButtonFormField<String> statusField =
        DropdownButtonFormField<String>(
    ...
    if (isCompact) {
      return Column(
        children: <Widget>[typeField, const SizedBox(height: 12), statusField],
      );
    }
  ...
  Widget _buildPostFields({required bool isCompact}) {
    final Widget ctaFields = isCompact
        ? Column(
            children: <Widget>[
              TextFormField(
      ...
      children: <Widget>[
        TextFormField(
          ...
          validator: (String? value) {
            ...
            if (text.length < 30) return 'Aggiungi almeno 30 caratteri';
            return null;
          },
        ),
        const SizedBox(height: 14),
  ...
  Widget _buildFooterActions({required bool isCompact, required bool isEdit}) {
    if (isCompact) {
      return Column(
  ...
  bool get _isPostType => _type == 'post';
...
class _MediaSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        border: Border.all(color: contentMediaBorderColor),
        borderRadius: BorderRadius.circular(16),
        color: contentMediaSurfaceColor,
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
  ...
  Widget _buildUrlRow() {
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) {
        const String labelText = 'Aggiungi URL immagine';
        const String hintText = 'https://cdn.esempio.it/file.jpg';

        final Widget urlField = TextFormField(
  ...
  String get _sectionTitle => switch (type) {
    'image' => 'Media immagine',
    'video' => 'Media video',
    _ => 'Media allegati',
  };

  String get _uploadButtonLabel => switch (type) {
    'video' => 'Carica uno o più video dal PC',
    'image' => 'Carica una o più immagini dal PC',
    _ => 'Carica media dal PC',
  };
}
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: contentPreviewBorderColor),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
    ...
    if (isVideo && _isHttpUrl(media.thumbnailReference)) {
      return Stack(
        ...
        children: <Widget>[
          ...
          const DecoratedBox(
            decoration: BoxDecoration(color: contentScrimColor),
          ),
          const Center(
            child: Icon(
